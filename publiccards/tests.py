...
class BusinessCardPrintTests(APITestCase):
    def test_print_view_can_render_promotional_flyer_layout(self):
        ...
        self.assertContains(response, "Volante promocional")
        self.assertContains(response, "La Costura de Dona Nancy")
        self.assertContains(response, "Escanea aqui", count=2)
        self.assertContains(response, "Conoce nuestra tarjeta digital y contactanos.", count=2)
        self.assertContains(response, "Nuestros servicios", count=2)
